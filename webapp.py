...
with st.form("form_mediciones", clear_on_submit=True):
    ...
    with col_der:
        ...
        res_u = st.text_input("U (Ω)")
        res_v = st.text_input("V (Ω)")
        res_w = st.text_input("W (Ω)")
        st.write("")
        st.markdown("### Aislamiento")
        mot_u_v = st.text_input("U-V (MΩ)")
        mot_u_w = st.text_input("U-W (MΩ)")
        mot_v_w = st.text_input("V-W (MΩ)")
        mot_u_t = st.text_input("U-T (MΩ)")
        mot_v_t = st.text_input("V-T (MΩ)")
        mot_w_t = st.text_input("W-T (MΩ)")
        dar = st.text_input("DAR")
        st.write("")
        st.markdown("### Corriente de servicio")
